class TetrisBattle {
    constructor() {
        ...
        // === 追加: シールド関連の変数 ===
        this.shieldUsesRemaining = 0; // The number of times the player can block attacks
        // === ここまで ===

        // Character selection
        this.selectedCharacter = 'char1'; // Default character
        this.characterImages = { // Map character IDs to image URLs
            'char1': './images/syado.png',
            'char2': './images/down1.png',
            'char3': './images/attack.png',
    ...
    handleServerMessage(message) {
        ...
        switch (message.type) {
            ...
            case 'opponentUpdate':
                ...
                break;
            case 'attack':
                console.log("攻撃メッセージ受信", message); // 追加: 攻撃メッセージの受信ログ
            ...
            case 'hideOpponentBoard':
                ...
                break;
            case 'defenceShield':
                this.defenceShield(message.shieldnum || 2);
                break;
    ...
    returnToHome() {
        ...
        // タイトル画像を再表示
        const titleImg = document.getElementById('titleImage');
        if (titleImg) titleImg.style.display = '';
    ...
    updateOpponent(data) {
        ...
        if (this.elements.opponentLevel) {
    ...
    activateSkill() {
        if (this.selectedCharacter === 'char1' && this.skillReady) {
            ...
            this.skillReady = false;
            this.updateSkillGaugeDisplay();
        } else if (this.selectedCharacter === 'char2' && this.skillReady) {
            // 他キャラのスキルは従来通り（必要ならここに追加）
            this.sendMessage('defenceShield', { shieldnum: 2 }); // シールドは２枚
            this.skillGauge = 0;
            this.skillReady = false;
            this.updateSkillGaugeDisplay();
        }
    ...
    sendAttack(lines, type) {
        console.log("sendAttack呼び出し", lines, type); // 追加: sendAttack呼び出しログ
        if (lines > 0) {
    ...
    hideOpponentBoard(duration) {
        // 自分の盤面のストライプオーバーレイを取得
        const overlay = document.querySelector('.player-section .stripeOverlay');
        if (!overlay) return;
        overlay.classList.remove('animate-stripes');
        void overlay.offsetWidth;
        overlay.classList.add('animate-stripes');
        setTimeout(() => {
            overlay.classList.remove('animate-stripes');
        }, duration);
    }
    ...
    //追加//
    /**
     * @function defenceShield
     * @description Grants the player a number of shield uses to block incoming garbage lines.
     * @param {number} shieldnum - The number of shield uses to grant.
     */
    defenceShield(shieldnum) {
        this.shieldUsesRemaining += shieldnum; // 受け取ったシールド数を加算
        console.log(`シールドを ${shieldnum} 枚獲得しました！現在のシールド数: ${this.shieldUsesRemaining}枚`);
        this.displayAttackIndicator(shieldnum, 'シールド獲得！', true); // シールド獲得の表示
    }
